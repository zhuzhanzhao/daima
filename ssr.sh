...
#	Author: 失落的梦
#	qq:1150315739
#	可联系TG：https://https://t.me/vpsjiaoliudajian 
#       博客2：https://www.vcxm789.asia
#       博客1：https://www.kehu33.asia
#=================================================
...
Set_password(){
	...
	read -e -p "(默认: 123456):" ss_password
	[[ -z "${ss_password}" ]] && ss_password="123456"
	echo && echo "========================"
...
Set_cipher(){
 ...
 ${Green_font_prefix}12.${Font_color_suffix} chacha20-ietf-poly1305 (AEAD)推荐
...
启用详细日志模式就可以在日志中看到链接者信息(链接时间、链接代理端口、链接者IP、链接者访问的目标域名或IP这些非敏感类信息)。"
	read -e -p "(默认：Y 启用):" ss_verbose
	[[ -z "${ss_verbose}" ]] && ss_verbose="Y"
...
Uninstall(){
	...
	if [[ ${unyn} == [Yy] ]]; then
		...
		if [[ ! -z $(crontab -l | grep "s.sh monitor") ]]; then
...
Set_crontab_monitor(){
	check_crontab_installed_status
	crontab_monitor_status=$(crontab -l|grep "s.sh monitor")
...
crontab_monitor_cron_start(){
	...
	sed -i "/s.sh monitor/d" "$file_1/crontab.bak"
	echo -e "\n* * * * * /bin/bash $file_1/s.sh monitor" >> "$file_1/crontab.bak"
	crontab "$file_1/crontab.bak"
	rm -r "$file_1/crontab.bak"
	cron_config=$(crontab -l | grep "s.sh monitor")
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "http://ww.qfqf.com/s.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
...
————————————
  ...
  ---- 微信：Falltoher-1314 作者：失落的梦 ----" && echo
	if [[ -e ${FILE} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
