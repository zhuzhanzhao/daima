...
install_acme(){
    ...
    if [[ -n $(~/.acme.sh/acme.sh -v 2>/dev/null) ]]; then
        ...
        green "建议如下："
        yellow "1. 检查VPS的网络环境"
        yellow "2. 脚本可能跟不上时代, 建议建议联系作者询问或截图发布到QQ群 微信群、或TG群询问"
    fi
    back2menu
}
...
acme_standalone(){
    ...
    if [[ -n $(echo $domainIP | grep nginx) ]]; then
        ...
        if [[ $domainIP != $ipv4 ]] && [[ $domainIP != $ipv6 ]]; then
            ...
            yellow "3. 脚本可能跟不上时代, 建议建议联系作者询问或截图发布到QQ群 微信群、或TG群询问"
            exit 1
        fi
    fi
...
uninstall() {
    [[ -z $(~/.acme.sh/acme.sh -v 2>/dev/null) ]] && yellow "未安装shell脚本, 卸载程序无法执行!" && exit 1
...
menu() {
    ...
    echo "#######################################################################################"
    echo ""
    echo -e " ${GREEN}1.${PLAIN} 安装 域名证书申请脚本"
